...
const namesOfWhoIsInSpace =  (callback)=> {
    const getRequest =  fetch('http://api.open-notify.org/astros.json')
        .then(response => response.json())
        .then(data => callback(data.people.map((person) => {
            return person.name
        })));
}
...
namesOfWhoIsInSpace(logger)

namesOfWhoIsInSpace(function(data){
    console.log(data)
})